...
// @version        0.10-beta
// @description    Toolbox per cassetto.agenziaentrate.gov.it: download massivo F24/F23/CU/Dich.Intento, Report Excel, supporto cassetto proprio e delegato
// @author         denvermotel
...
// resta `0.11-alpha` e i manifest `0.11.0`, che e' la forma pretesa dagli store.
var VERSION = '0.10β';
...
var isIntentoList, isIntentoDetail;
...
function detectContext() {
    ...
    // Dichiarazioni di intento (Ric=DEN, dichiarante o destinatario)
    isIntentoList   = /[?&]Ric=DEN(&|$)/.test(h) && h.indexOf('Protocollo=') === -1;
    isIntentoDetail = /[?&]Ric=DEN(&|$)/.test(h) && h.indexOf('Protocollo=') !== -1;
    ...
    isListPage  = isF24List || isF23List || isCUList || isIntentoList;
    isDetailPage = isF24Detail || isF23Detail || isCUDetail || isF24SearchDetail || isIntentoDetail;
    isVersPage  = /[?&]Ric=VERS(&|$)/.test(h) && !isListPage && !isDetailPage;
    docType     = isCUList || isCUDetail ? 'CU'
                : (isIntentoList || isIntentoDetail ? 'Dich. Intento'
...
function foglioStile() {
    return [
        ...
        /* Chi stampa una schermata del cassetto sta facendo un documento per
           un cliente o per un fascicolo: la barra non c'entra niente, e in
           cima al foglio si porterebbe via un centimetro buono.

           Queste righe bastano per la linguetta, i dialoghi e il selettore
           delle date, che il display se lo prendono da qui. Non bastano per
           #CU_Panel: la barra si apre scrivendo `display:block!important`
           sull'elemento, e una dichiarazione inline con !important batte
           qualunque regola di foglio, @media print compreso. La barra e lo
           scosto del body si tolgono percio' in `preparaStampa()`, a mano.
           E' l'errore della prima stesura di questo fix: la regola c'era, ma
           la barra si stampava lo stesso. */
        '@media print{',
        '#CU_Panel,#CU_Linguetta,#CU_AlertOverlay,#CU_DateSel{display:none!important;}}'
    ].join('');
}
...
function aggiornaPadding() {
    if (!_barraVisibile || !pannello || _inStampa) return;
    document.body.style.setProperty('padding-top', (pannello.offsetHeight || 34) + 'px', 'important');
}

/*
 * In stampa la barra sparisce per foglio di stile, ma lo scosto del body no:
 * e' scritto inline e con !important, e una regola @media print non lo batte -
 * inline batte tutto. Quindi lo si toglie a mano e lo si rimette dopo.
 *
 * La guardia dentro aggiornaPadding() non e' pignoleria: nascondere il
 * pannello ne cambia l'altezza, il ResizeObserver scatta, e siccome un
 * elemento nascosto misura zero il ripiego `|| 34` rimetterebbe trentaquattro
 * pixel di bianco proprio mentre si sta stampando.
 *
 * Due modi per accorgersene, perche' non c'e' un modo solo che vada ovunque:
 * beforeprint/afterprint su Chrome, Firefox ed Edge, il cambio di
 * matchMedia('print') dove quegli eventi non arrivano.
 */
var _inStampa = false;

function preparaStampa(inStampa) {
    if (_inStampa === inStampa) return;
    _inStampa = inStampa;
    if (inStampa) {
        document.body.style.removeProperty('padding-top');
        if (pannello) pannello.style.setProperty('display', 'none', 'important');
    } else {
        // Solo se era aperta: la stampa non e' un modo per riaprirla
        if (pannello && _barraVisibile) pannello.style.setProperty('display', 'block', 'important');
        aggiornaPadding();
    }
}

function ascoltaStampa() {
    window.addEventListener('beforeprint', function() { preparaStampa(true); });
    window.addEventListener('afterprint', function() { preparaStampa(false); });
    if (typeof window.matchMedia !== 'function') return;
    var mm = window.matchMedia('print');
    var cambio = function(e) { preparaStampa(!!e.matches); };
    if (mm.addEventListener) mm.addEventListener('change', cambio);
    else if (mm.addListener) mm.addListener(cambio);   // Safari di qualche anno fa
}
...
function rebuildButtons() {
    ...
    if (isF24List) {
        ...
        h = '<button class="cuBtn cu-primario" data-action="downloadAllCU">Scarica CU</button>' +
            ...
            '<button class="cuBtn cu-quieto" data-action="summary">Riepilogo</button>';
    } else if (isIntentoList) {
        h = '<button class="cuBtn cu-primario" data-action="downloadAllIntento">Scarica Dich. Intento</button>' +
            '<button class="cuBtn cu-azione" data-action="reportExcelIntento">Report Excel Dich. Intento</button>' +
            '<button class="cuBtn cu-quieto" data-action="summary">Riepilogo</button>';
    ...
    } else if (isIntentoDetail) {
        h = '<button class="cuBtn cu-primario" data-action="downloadIntentoPdf">Genera PDF Dich. Intento</button>';
        ...
        h = '<button class="cuBtn cu-quieto" data-action="goToVers">Versamenti</button>' +
            ...
            '<button class="cuBtn cu-quieto" data-action="goToIntento">Dichiarazioni di intento</button>';
    }
...
/* ═══════════════════════════════════════════════════════════════
   DICHIARAZIONI DI INTENTO — COLLECT, DOWNLOAD, EXCEL
   ═══════════════════════════════════════════════════════════════ */

function collectIntento() {
    var items = [];
    document.querySelectorAll('a[href*="Ric=DEN"][href*="Protocollo="]').forEach(function(a) {
        var li = a.closest('li'); if (!li) return;
        var href = a.getAttribute('href');
        var protocollo = getParam('Protocollo', href);
        var anno = getParam('Anno', href);
        var numDich = a.textContent.trim().replace(/\s+/g, ' ');
        var corrSpan = li.querySelector('span.correlato');
        var altroSoggetto = '';
        if (corrSpan) { altroSoggetto = corrSpan.childNodes[0] ? corrSpan.childNodes[0].textContent.trim() : corrSpan.textContent.trim(); }

        // Il ruolo sta nell'<h3> della sezione, fratello del .card che
        // contiene la lista (non della lista stessa). Il titolo pagina è il ripiego.
        var ruolo = 'destinatario';
        var card = li.closest('.card');
        var prevEl = card ? card.previousElementSibling : null;
        while (prevEl && prevEl.tagName !== 'H3') prevEl = prevEl.previousElementSibling;
        var etichettaRuolo = prevEl ? prevEl.textContent : (document.getElementById('titolo') || {}).textContent;
        if (etichettaRuolo && /dichiarante/i.test(etichettaRuolo)) ruolo = 'dichiarante';

        items.push({ href:href, protocollo:protocollo, anno:anno, numDich:numDich, altroSoggetto:altroSoggetto, ruolo:ruolo });
    });
    return items;
}

/* URL del PDF («Formato stampabile» del dettaglio) */
function intentoPdfUrl(anno, protocollo) {
    return SERVLET+'?Ric=DEN&Anno='+encodeURIComponent(anno)+'&Protocollo='+encodeURIComponent(protocollo)+'&Quadro=&stampa=P';
}

/* Fetch del dettaglio per dichiarante, destinatario e dati dichiarazione.
   Ricerca per etichetta, come per la CU: nessuna classe distintiva nel DOM. */
function fetchIntentoDettaglio(anno, protocollo) {
    var url = SERVLET+'?Ric=DEN&Anno='+encodeURIComponent(anno)+'&Protocollo='+encodeURIComponent(protocollo)+'&Quadro=';
    var vuoto = { dichiarante:'', dichiaranteCF:'', destinatario:'', destinatarioCF:'', caselladogana:'', tipoOperazione:'', annoRiferimento:'', operazioneSingola:'', plafond:'' };
    return fetch(url, {credentials:'include'}).then(function(r){return r.text();}).then(function(html) {
        var doc = new DOMParser().parseFromString(html, 'text/html');

        function cardBody(titolo) {
            var h4s = doc.querySelectorAll('h4');
            for (var i = 0; i < h4s.length; i++) {
                if (h4s[i].textContent.trim().toLowerCase() === titolo.toLowerCase()) {
                    var card = h4s[i].nextElementSibling;
                    return card ? card.querySelector('.card-body') : null;
                }
            }
            return null;
        }
        function testo(body) { return body ? body.textContent.replace(/\s+/g,' ').trim() : ''; }
        function cfOPiva(txt) {
            var cf = txt.match(/Codice fiscale\s*([A-Z0-9]{11,16})/i);
            var piva = txt.match(/Partita IVA\s*(\d{11})/i);
            return (cf && cf[1]) ? cf[1] : ((piva && piva[1]) ? piva[1] : '');
        }
        function denominazione(txt) {
            var m = txt.match(/^(.*?)\s*Codice fiscale/i);
            return (m ? m[1] : txt).trim();
        }

        var bodyDichiarante   = cardBody('Dati del dichiarante');
        var bodyDestinatario  = cardBody('Destinatario della dichiarazione');
        var txtDichiarante    = testo(bodyDichiarante);
        var txtDestinatario   = testo(bodyDestinatario);
        var txtDichiarazione  = testo(cardBody('Dichiarazione'));

        /*
         * Nel quadro «Destinatario della dichiarazione» la casella dogana e la
         * denominazione sono due <b> distinti in ordine fisso (dogana, poi
         * denominazione): sul testo appiattito finirebbero incollati.
         */
        var bDest = bodyDestinatario ? bodyDestinatario.querySelectorAll('b') : [];
        var caselladogana = bDest[0] ? bDest[0].textContent.replace(/\s+/g,' ').trim() : '';
        var destinatarioDenom = bDest[1] ? bDest[1].textContent.replace(/\s+/g,' ').trim() : denominazione(txtDestinatario);

        var tipoOperazione = /IMPORTAZIONI/i.test(txtDichiarazione) ? 'IMPORTAZIONI'
                            : (/ACQUISTI/i.test(txtDichiarazione) ? 'ACQUISTI' : '');
        var mAnno = txtDichiarazione.match(/ANNO\s*(\d{4})/i);
        var mSingola = txtDichiarazione.match(/una sola operazione per un importo fino a euro\s*([\d.,]+)/i);
        var mPlafond = txtDichiarazione.match(/operazioni fino a concorrenza di euro\s*([\d.,]+)/i);

        return {
            dichiarante: denominazione(txtDichiarante), dichiaranteCF: cfOPiva(txtDichiarante),
            destinatario: destinatarioDenom, destinatarioCF: cfOPiva(txtDestinatario), caselladogana: caselladogana,
            tipoOperazione: tipoOperazione, annoRiferimento: mAnno ? mAnno[1] : '',
            operazioneSingola: mSingola ? mSingola[1] : '', plafond: mPlafond ? mPlafond[1] : ''
        };
    }).catch(function() { return vuoto; });
}

var intentoDlLog = [];

async function runBatchIntento(items) {
    if (!items.length) { showSt('Nessuna dichiarazione di intento trovata.', 0, 'avviso'); concludiReport(true); return; }
    if (!await confermaLotto(items.length, 'dichiarazioni di intento')) {
        showSt('Download annullato.', 0, 'avviso'); setTimeout(hideSt, 3000); return;
    }
    setDis(true); intentoDlLog = [];
    var ok_n = 0, fail_n = 0;
    for (var i = 0; i < items.length; i++) {
        var it = items[i];
        var fname = safe(getIdentifier().code)+'_'+safe(it.anno)+'_Dich_Intento_'+safe(it.altroSoggetto)+'_idx'+i+'.pdf';
        showSt('('+(i+1)+'/'+items.length+') '+fname, Math.round(i/items.length*100));
        var ok = await dlPdf(intentoPdfUrl(it.anno, it.protocollo), fname);
        intentoDlLog.push({numDich:it.numDich, protocollo:it.protocollo, ruolo:it.ruolo, altroSoggetto:it.altroSoggetto, filename:fname, ok:ok});
        if (ok) ok_n++; else fail_n++;
        await sleep(600);
    }
    showSt('Completato: '+ok_n+' scaricate'+(fail_n>0?', '+fail_n+' non riuscite':'')+'. Il report Excel Dich. Intento ne fa il riepilogo.', 100, fail_n>0?'avviso':'riuscito');
    setDis(false);
    concludiReport(fail_n > 0);
}

/* Report Excel — una riga per dichiarazione, con lo stato dello scarico se
   un lotto è già stato eseguito. */
async function reportExcelIntento() {
    var items = collectIntento();
    if (!items.length) { showSt('Nessuna dichiarazione di intento trovata.', 0, 'avviso'); concludiReport(true); return; }
    setDis(true);
    var data = [];
    for (var i = 0; i < items.length; i++) {
        var it = items[i];
        showSt('Lettura dati ('+(i+1)+'/'+items.length+')…', Math.round(i/items.length*100));
        var det = await fetchIntentoDettaglio(it.anno, it.protocollo);
        var logEntry = intentoDlLog.find(function(l){return l.protocollo === it.protocollo;});
        var stato = logEntry ? (logEntry.ok ? 'Scaricato' : 'Errore') : 'Non scaricato';
        data.push({
            n: i+1, numDich: it.numDich, anno: it.anno, protocollo: it.protocollo, ruolo: it.ruolo,
            dichiarante: det.dichiarante, dichiaranteCF: det.dichiaranteCF,
            destinatario: det.destinatario, destinatarioCF: det.destinatarioCF, caselladogana: det.caselladogana,
            tipoOperazione: det.tipoOperazione, annoRiferimento: det.annoRiferimento,
            operazioneSingola: det.operazioneSingola, plafond: det.plafond,
            filename: logEntry ? logEntry.filename : '', stato: stato
        });
        await sleep(300);
    }
    var cod = getIdentifier().code;
    ...
    var totERR = data.filter(function(d){return d.stato === 'Errore';}).length;
    var totNS = data.filter(function(d){return d.stato === 'Non scaricato';}).length;
    var totDich = data.filter(function(d){return d.ruolo === 'dichiarante';}).length;
    var totDest = data.filter(function(d){return d.ruolo === 'destinatario';}).length;

    var hdrs = ['N°','N. Dichiarazione','Anno','Protocollo','Ruolo',
        'Dichiarante','CF/P.IVA Dichiarante','Destinatario','CF/P.IVA Destinatario','Casella dogana',
        'Tipo operazione','Anno riferimento','Operazione singola','Plafond',
        'Nome File','Stato'];
    var hrow = hdrs.map(function(h){return cella(h,'String','hdr');});
    var widths = [35,200,60,220,95,220,140,220,140,100,110,90,120,120,280,90];
    var drows = data.map(function(d) {
        var sty = d.stato === 'Scaricato' ? 'ok' : (d.stato === 'Errore' ? 'er' : 'wn');
        return [cella(d.n,'Number',sty),cella(d.numDich,'String',sty),cella(d.anno,'String',sty),cella(d.protocollo,'String',sty),
            cella(d.ruolo,'String',sty),cella(d.dichiarante,'String',sty),cella(d.dichiaranteCF,'String',sty),
            cella(d.destinatario,'String',sty),cella(d.destinatarioCF,'String',sty),cella(d.caselladogana,'String',sty),
            cella(d.tipoOperazione,'String',sty),cella(d.annoRiferimento,'String',sty),
            cellaImporto(d.operazioneSingola,sty),cellaImporto(d.plafond,sty),
            cella(d.filename,'String',sty),cella(d.stato,'String',sty)];
    });
    var sh1 = { nome:'Elenco Dich. Intento', colonne:widths, righe:[hrow].concat(drows) };

    var sh2 = { nome:'Riepilogo Dich. Intento', colonne:[220,180], righe:[
        [cella('Cassetto_Utility v'+VERSION+' — Report Dichiarazioni di intento','String','ttl')],
        [cella('Identificativo:'),cella(cod)],
        [cella('Modalità:'),cella(tipoCod)],
        [cella('Anno imposta:'),cella(anno)],
        [cella('Data report:'),cella(nowStr)],
        [],
        [cella('Dichiarazioni trovate:','String','bld'),cella(items.length,'Number')],
        [cella('Come dichiarante:','String','bld'),cella(totDich,'Number')],
        [cella('Come destinatario:','String','bld'),cella(totDest,'Number')],
        [],
        [cella('Scaricate:','String','bld'),cella(totOK,'Number','ok')],
        [cella('Errori:','String','bld'),cella(totERR,'Number','er')],
        [cella('Non scaricate:','String','bld'),cella(totNS,'Number','wn')]
    ...
    dlXLS([sh1, sh2], safe(cod)+'_'+safe(anno)+'_ReportDichIntento_CassettoUtility.xlsx');
    showSt('Report Excel generato: ' + data.length + ' dichiarazioni.', 100, 'riuscito');
    setDis(false); concludiReport(false);
}
...
async function eseguiAzione(action) {
    ...
    if (action === 'goToIntento') { location.href = SERVLET+'?Ric=DEN'; return; }
    ...
    if (action === 'summary') {
        ...
        if (isCUList) {
            ...
            var intentoItems = collectIntento();
            showSt('Dichiarazioni di intento \u2014 totale: '+intentoItems.length+(intentoDlLog.length ? ' | Scaricate: '+intentoDlLog.filter(function(l){return l.ok;}).length : ''), 100);
        } else {
    ...
    // Batch download Dichiarazioni di intento
    if (action === 'downloadAllIntento') { await runBatchIntento(collectIntento()); return; }

    // Report Excel Dichiarazioni di intento
    if (action === 'reportExcelIntento') { await reportExcelIntento(); return; }
    ...
    // Dettaglio Dichiarazione di intento — Genera PDF
    if (action === 'downloadIntentoPdf') {
        var curUrl3 = location.href;
        var annoI = getParam('Anno', curUrl3), protI = getParam('Protocollo', curUrl3);
        var fname3 = safe(getIdentifier().code)+'_'+safe(annoI)+'_Dich_Intento_'+safe(protI.slice(-7))+'.pdf';
        showSt('Generazione del PDF…', 50); setDis(true);
        var ok3 = await dlPdf(intentoPdfUrl(annoI, protI), fname3);
        showSt(ok3 ? fname3+' generato.' : 'Generazione del PDF non riuscita.', 100, ok3?'riuscito':'errore');
        setDis(false); concludiReport(!ok3); return;
    }
}
...
var COMANDI_MENU = ['mostraBarra', 'goToVers', 'goToF24', 'goToF23', 'goToCU', 'goToIntento'];
